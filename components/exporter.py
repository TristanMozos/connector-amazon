...
from .backend_adapter import AMAZON_DATETIME_FORMAT

_logger = logging.getLogger(__name__)

"""

Exporters for Amazon.

In addition to its export job, an exporter has to:

* check in Amazon if the record has been updated more recently than the
...
class AmazonExporter(AbstractComponent):
    ...
    def _should_import(self):
        ...
        in Amazon and the last sync date in Odoo,
        if the former is more recent, schedule an import
        to not miss changes done in Amazon.
        """
        assert self.binding
        ...
        amazon_date = datetime.strptime(record['updated_at'],
                                        AMAZON_DATETIME_FORMAT)
        return sync_date < amazon_date
