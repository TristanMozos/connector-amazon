...
class MWS(object):
    ...
    def make_request(self, extra_data, method="GET", **kwargs):
        ...
        try:
            pool.use_quota_if_avaiable(env, request_name=params.get('Action'), backend_id=self._backend.id)
    ...
    def calc_signature(self, method, request_description):
        ...
        return base64.b64encode(
            hmac.new(
                self._backend.key.encode(), sig_data.encode(), hashlib.sha256
            ).digest()
        )
...
class Products(MWS):
    ...
    def get_matching_product_for_id(self, marketplaceid, type_, ids):
        ...
        """
        data = dict(
            Action="GetMatchingProductForId", MarketplaceId=marketplaceid, IdType=type_
        )
    ...
    def get_product_categories_for_asin(self, marketplaceid, asin):
        data = dict(
            Action="GetProductCategoriesForASIN", MarketplaceId=marketplaceid, ASIN=asin
        )
        return self.make_request(data)
...
class InboundShipments(MWS):
    ...
    LABEL_PREFERENCES = ["SELLER_LABEL", "AMAZON_LABEL_ONLY", "AMAZON_LABEL_PREFERRED"]
    ...
    def get_transport_content(self, shipment_id):
        ...
        inbound shipment.
        """
        data = dict(Action="GetTransportContent", ShipmentId=shipment_id)
        return self.make_request(data, method="POST")
    ...
    def void_transport_request(self, shipment_id):
        ...
        """
        data = dict(Action="VoidTransportRequest", ShipmentId=shipment_id)
        return self.make_request(data, method="POST")
